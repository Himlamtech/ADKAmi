...
def format_string(doc: Dict[str, Any]) -> str:
    ...
    if content := doc.get('content', '').strip():
        ...
        parts.append(f"Chi tiết: {content}")
    ...
    if keywords := doc.get('keywords'):
        ...
        if keywords_str:
            parts.append(f"Tag: {keywords_str}")
...
def format_search_result(doc: Dict[str, Any]) -> str:
    ...
    # Main concept
    if header := doc.get('header', '').strip():
        result.append(f"🎯 **Đề mục:** {header}")
    
    # Definition (shortened)
    if content := doc.get('content', '').strip():
        content = content[:200] + "..." if len(content) > 200 else content
        result.append(f"📝 **Nội dung:** {content}")
...
def format_document_for_context(doc: Dict[str, Any]) -> str:
    ...
    if header := doc.get('header', '').strip():
        context_parts.append(f"**Đề mục:** {header}")
...
def find_similar_information_by_hybrid_search(
    query: str,
    limit: int = 10,
    candidates: int = 50,
    ...
    """
    Đây là một tool tìm kiếm hybrid để tìm kiếm các thông tin tuyển sinh tương tự sử dụng kết hợp vector search và text search.
    Sử dụng tool này khi cần tìm kiếm các thông tin tuyển sinh liên quan đến một chủ đề hoặc câu hỏi cụ thể,
    với khả năng tìm kiếm ngữ nghĩa (vector) và tìm kiếm văn bản (text) được thực hiện song song.

    Parameters:
        query (str): Câu truy vấn tìm kiếm thông tin tuyển sinh.
                    Có thể là từ khóa, câu hỏi hoặc mô tả về nội dung thông tin tuyển sinh cần tìm.
                    Ví dụ: "đồ thị", "lý thuyết đồ thị", "đồ thị có hướng".
        limit (int, optional): Số lượng thông tin tuyển sinh tối đa trả về. Mặc định là 10.
    ...
    Returns:
        dict: Một từ điển chứa kết quả tìm kiếm với các key:
            ...
            - 'data': danh sách các thông tin tuyển sinh tìm được dưới dạng chuỗi formatted (chỉ khi thành công)
            - 'message': thông báo về kết quả hoặc lỗi
    """
    try:
        collection = admission_mongo_client['admin_data']['admission']
        embedding_query = get_embedding(query, DEFAULT_EMBEDDING_MODEL)
        ...
        pipeline = [
            {
                "$search": {
                    ...
                    "text": {
                        "query": query,
                        "path": ["keywords", "content", "header"],
                        "synonyms": "synonym_mapping" # synonyms mapping 
                    }
                }
            },
            {
